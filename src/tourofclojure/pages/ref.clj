...
(defn desc
  ...
  (alter account2 #(- % 100)))"]]
          ...
          [:p "Si l'ordre dans lequel s'effectue les opérations n'a pas d'importance, il est alors possible d'utiliser " [:b "compute"]]
          [:pre [:code "(dosync
  ...
  (ref-set account2 0))"]]
          [:h3 "Attention"]
          [:p "Les transactions pouvant potentiellement se relancer plusieurs fois, leur utilisation n'est pas recommendée pour les opérations qui provoquent des effets de bord (I/O)."]]
